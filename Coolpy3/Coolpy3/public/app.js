(function ($) {
    ...
    function appvm() {
        ...
        self.bt_upmenu = function () {
            $.AMUI.progress.start();
            $.ajax({
                url: "/v1.0/user/wc/updatemenu?username=" + $.AMUI.utils.cookie.get('uid') + "&pass=" + $.AMUI.utils.cookie.get('pwd'), 
            }).done(function (data) {
                if (data === "") {
                    $("#msg").html("微信菜单更新成功！");
                    $('#my-prompt').modal('open');
        ...
        if (wcid !== null) {
            $.ajax({
                url: "/v1.0/user?username=" + wcid + "&pass=" + wcid
            }).done(function (data) {
                if (data.ukey !== null) {
                    $.AMUI.utils.cookie.set('ukey', data.ukey);
                    $.AMUI.utils.cookie.set('uid', wcid);
                    $.AMUI.utils.cookie.set('pwd', wcid);
                    if (nav !== null) {
                        go("web/" + nav + ".html");
                        $("#mainModel").removeClass("am-hide");
                        $("header").addClass("am-hide");
                    }
                } else {
                    $("#msg").html("登陆失败");
                    $('#my-prompt').modal('open');
